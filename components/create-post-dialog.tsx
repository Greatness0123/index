...
export async function createCommunityPost(formData: FormData) {
  ...
  try {
    ...
    // First, check if the community_posts table exists
    ...
    if (tableCheckError && tableCheckError.code === '42P01') {
      console.error("Community posts table doesn't exist:", tableCheckError)
      return { 
        error: "Community features are not configured. Please contact support." 
      }
    }

    // Insert the post with basic fields
    const postData: any = {
      title: title.trim(),
      content: content.trim(),
    ...
    if (authorName) postData.author_name = authorName
    if (externalUrl) postData.external_url = externalUrl.trim()
    if (tagsArray.length > 0) postData.tags = tagsArray
    if (imageUrlArray.length > 0) postData.all_images = imageUrlArray
    ...
    if (error) {
      ...
      if (error.code === '42501') {
        return { error: "Permission denied. Please check if you have the right to create posts." }
      }
      
      return { error: error.message || "Failed to create post. Please try again." }
    }

    // Handle multiple images in post_images table if it exists
    if (imageUrlArray.length > 0 && data) {
      try {
        // Check if post_images table exists
        const { error: imageTableCheck } = await supabaseClient
          .from("post_images")
          .select("id")
          .limit(1)
        ...
        console.error("Error handling post images:", imageError)
      }
    }
    ...
    console.error("Unexpected error creating community post:", error)
    return { error: "An unexpected error occurred. Please try again." }
  }
}
